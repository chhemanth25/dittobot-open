...
class Extras(commands.Cog):
    ...
    async def status(self, interaction):
        ...
        clusternum = 1
        shardnum = len(self.ctx.bot.shards)

        result = await self.ctx.bot.handler("num_processes", 1, scope="launcher")

        if result:
            clusternum = result[0]["clusters"]
            shardnum = result[0]["shards"]
            process_res = await self.ctx.bot.handler(
                "_eval",
                clusternum,
            ...
            clusternum = "1"
            shardnum = len(self.ctx.bot.shards)
            servernum = len(self.ctx.bot.guilds)

        embed.add_field(
            name="Bot Information",
            value=(
                f"`Owner/Founders:` **{self.ctx.bot.owner.name}, Eaaarl#3381, Chichiri12345#6662, Cruithne#1421**\n"
                "`Developers:` **Cruithne#1421, Motzumoto#9773, Zak~#0193**\n"
                "`Server Host:` **Krypt**\n"
        ...
        async def creditcallback(interaction):
            await self.credit_page(self.ctx)
            await interaction.response.edit_message(content=embed, view=view)
    ...
    async def claim(self, ctx):
        ...
        async with self.ctx.bot.db[0].acquire() as pconn:
            points = await pconn.fetchval(
                "SELECT upvotepoints FROM users WHERE u_id = $1", self.ctx.author.id
            )
            if points is None:
                await self.ctx.send(f"You have not Started!\nStart with `/start` first!")
                return
            if points < 5:
                await self.ctx.send("You do not have enough Upvote Points for your rewards.")
                return
            await pconn.execute(
        ...
        await self.ctx.send("Upvote Points Claimed!")
    ...
    async def vote(self, ctx):
        ...
        embed.description = (
            ...
            "<a:vote1:1013434144023380078><a:vote2:1013434140877664256> -> [#1 top.gg CLICK HERE TO VOTE](https://top.gg/bot/1000125868938633297/vote)\n"
        ...
        # link = [
        # "[Medium Cafe -  discord.gg/draw](https://discord.gg/P8ub5zBe29)",
        # "[Koma Cafe - discord.gg/cafe](https://discord.gg/VFQDGyD3vs)",
        # "[Nezuko - discord.gg/shiba](https://discord.gg/GC2SgbvVm4)",
        # "[Yume - discord.gg/yume](https://discord.gg/65USpZGun3)",
        # "[Weeb Kingdom - discord.gg/wk](https://discord.gg/2xqgt8QCg2)",
        # ]
        # final_link = random.choice(link)
        #embed.add_field(name="DittoBOT is Verified and on TopGG now!!", value=f"Vote-rewards for streaks will be updated soon! Thank you everyone!", inline=False)
        embed.set_footer(
    ...
    async def predeem(self, ctx):
        ...
        if patreon_status == "Elite Patreon":
            ...
            amount = 15
        elif patreon_status == "MewBot Patreon":
            amount = 3
        else:
            await ctx.send(
        ...
        async with ctx.bot.db[0].acquire() as tconn:
            ...
            if is_staff.lower() != "user":
                embed.set_author(
                    ...
                    icon_url="https://cdn.discordapp.com/attachments/1004311737706754088/1005748208787529748/ezgif-4-23632dbfbb.gif",
                )

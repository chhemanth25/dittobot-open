...
from .misc import ExpiringEffect, ExpiringItem, ExpiringWish
...
class Trainer:
    ...
    def __init__(self, name: str, party: list):
        self.name = name
        self.party = party
        self.current_pokemon = party[0] if party else None
    ...
    def valid_swaps(self, defender, battle, *, check_trap=True):
        ...
        if check_trap and self.current_pokemon is not None:
            ...
            if (
                ...
                and self.current_pokemon.ability() != Ability.SHADOW_TAG
            ):
                return []
            if (
    ...
    def valid_moves(self, defender):
        ...
        for idx, move in enumerate(self.current_pokemon.moves):
            ...
            if move.damage_class == DamageClass.STATUS:
                if self.current_pokemon.held_item == "assault-vest":
                    continue
                if self.current_pokemon.taunt.active():
                    continue
            ...
            if (
                ...
                continue
            result.append(idx)
        return ("idxs", result) if result else ("struggle", [0, 1, 2, 3])

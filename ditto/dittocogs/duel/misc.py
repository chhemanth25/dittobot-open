...
class ExpiringEffect:
    ...
    def active(self):
        ...
        return True if self._remaining_turns is None else bool(self._remaining_turns)
    ...
    def set(self, weather: str, pokemon):
        ...
        if weather == "h-rain":
            ...
            if self._weather_type in ("h-rain", "h-sun", "h-wind"):
                return ""
            turns = 8 if pokemon.held_item == "icy-rock" else 5
            ...
            if self._weather_type in ("h-rain", "h-sun", "h-wind"):
                return ""
            turns = 8 if pokemon.held_item == "damp-rock" else 5
            ...
            if self._weather_type in ("h-rain", "h-sun", "h-wind"):
                return ""
            turns = 8 if pokemon.held_item == "smooth-rock" else 5
            ...
            if self._weather_type in ("h-rain", "h-sun", "h-wind"):
                return ""
            turns = 8 if pokemon.held_item == "heat-rock" else 5
...
class NonVolatileEffect:
    ...
    def next_turn(self, battle):
        ...
        if self.pokemon.ability() == Ability.HYDRATION and battle.weather.get() in (
            ...
            return self._extracted_from_next_turn_11("'s hydration cured its ")
        if self.pokemon.ability() == Ability.SHED_SKIN and not random.randint(0, 2):
            return self._extracted_from_next_turn_11("'s shed skin cured its ")
    ...
    # TODO Rename this here and in `next_turn`
    def _extracted_from_next_turn_11(self, arg0):
        removed = self.current
        self.reset()
        return f"{self.pokemon.name}{arg0}{removed}!\n"
    ...
    def apply_status(
        ...
        *,
        ...
        if status in ("poison", "b-poison"):
            ...
            ) and (attacker is None or attacker.ability() != Ability.CORROSION):
                return ""
            if self.pokemon.ability(attacker=attacker, move=move) in (
                Ability.IMMUNITY,
                Ability.PASTEL_VEIL,
...
class Metronome:
    ...
    def get_buff(self, movename):
        ...
        return 1 if self.move != movename else min(2, 1 + (0.2 * self.count))
...
class HeldItem:
    ...
    def __init__(self, item_data, owner):
        self.item = None if item_data is None else Item(item_data)
        self.owner = owner
        self.battle = None
        self.last_used = None
    ...
    def get(self):
        ...
        if self.owner.ability() == Ability.KLUTZ:
            return None
        return None if self.owner.corrosive_gas else self.item.name
    ...
    def should_eat_berry_status(self, otherpoke=None):
        ...
        if self in ("rawst-berry", "lum-berry") and self.owner.nv.burn():
            return True
        return bool(
            self in ("persim-berry", "lum-berry") and self.owner.confusion.active()
        )
